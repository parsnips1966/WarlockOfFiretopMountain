    ...
    if vars.decision_79 == "RING":
        ...
        if vars.dice_num <= vars.hero[1]:
            ...
            if vars.decision_84 == "TURN":
                ...
                vars.monster = [7, 6]
                fight("Crocodile", rounds=2)
                story("Your combined thrashings attract the 'turbulence' in the water\nthat you had noticed before and this now makes its way towards your part of the river.")
                ...
                if vars.decision_87 == "CONTINUE":
                    if fight("Crocodile"):
                        story("As you swim to the bank away from the crocodile\nyou look back to see the mysterious 'turbulence' approach the reptile's body,\nbreak into a frenzy and then move off, leaving not a trace of the animal.")
                        story("Thankful that you were not there to find out what it was,\nyou lift yourself out of the water onto the north bank")
                        change_stats(0, 1)
                        change_stats(2, 2)
                        vars.checkpoint = 13
                        pass
                    ...
                    if wounded:
                        ...
                        if vars.dice_num < 3:
                            ...
                            story("The majority of the Piranhas go for the Crocodile.")
                            vars.monster = [5, 1]
                    if fight("Piranhas"):
                        change_stats(2, 1)
                        vars.decision_88 = story("You swim to shore. Do you want to eat provisions?")
                        if vars.decision_88 == "YES":
                            take_provs()
                            vars.decision_88 = "NO"
                        vars.checkpoint = 13
            elif vars.decision_84 == "DETOUR":
                vars.piranhas = True
        else:
...
if vars.piranhas:
    ...
    vars.monster = [6, 5]
    if fight("Piranhas"):
        vars.decision_88 = story("You manage to scramble out of the water and lie panting\non the south bank. You may eat provisions here. Will you?")
        if vars.decision_88 == "YES":
            take_provs()
            vars.decision_88 = "NO"
